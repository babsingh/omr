...
/**
 * Determine if omrsig_protect is capable of protecting a function from the indicated signals in the indicated way. omrsig_protect only supports
 * synchronous signals so omrsig_can_protect does not need to check for asynchronous signals.
 *
 * @param[in] portLibrary The port library
 * @param[in] flags flags to control the signal handlers. Construct from bitwise-orring any of the following constants:
 ...
 *	OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION - indicates that the handler might return OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION
 *
 * If the signal option OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS has been set, then we cannot protect against any synchronous signals.
 *
 * Refer to the definition of the OMRPORT_SIG_FLAG_SIGALLSYNC macro for the list of asynchronous signals.
 *
 * @return non-zero if the port library can support the specified flags; OMRPORT_SIG_ERROR for a non-syncrhonous flag.
 */
int32_t
