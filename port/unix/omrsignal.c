...
/* Records the (port library defined) signals for which we have registered a handler.
 * Access to this must be protected by the registerHandlerMonitor.
 */
...
omrsig_set_options(struct OMRPortLibrary *portLibrary, uint32_t options)
{
	...
	if (0 != (options & OMRPORT_SIG_OPTIONS_ZOS_USE_CEEHDLR)) {
		...
		if (0 == (signalsWithHandlers & OMRPORT_SIG_FLAG_SIGALLSYNC)) {
