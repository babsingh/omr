...
omrsig_startup(struct OMRPortLibrary *portLibrary)
{
	...
	if (attachedPortLibraries++ == 0) {
		...
		for (; index < ARRAY_SIZE_SIGNALS; index++) {
...
asynchSignalReporter(void *userData)
{
	...
	while (0 == shutDownASynchReporter) {
		...
		for (; unixSignal < ARRAY_SIZE_SIGNALS; unixSignal++) {
			 ...
			 * even if some wake signals for the condvar are missed.
			 */
			for (; signalCount > 0; signalCount--) {
		...
		for (; unixSignal < ARRAY_SIZE_SIGNALS; unixSignal++) {
			uintptr_t signalCount = signalCounts[unixSignal];
			/* sem_wait will fall-through for each sem_post. So, we can
			 * handle one signal at a time. Ultimately, all signals will be
			 * handled.
			 */
			if (signalCount > 0) {
				asyncSignalFlag = mapUnixSignalToPortLib(unixSignal, NULL);
...
registerSignalHandlerWithOS(OMRPortLibrary *portLibrary, uint32_t portLibrarySignalNo, unix_sigaction handler, void **oldOSHandler)
{
	...
	if (0 == oldActions[unixSignalNo].restore) {
		...
		if (OMRSIG_SIGACTION(unixSignalNo, &newAction, &oldActions[unixSignalNo].action)) {
			...
			Trc_PRT_signal_registerSignalHandlerWithOS_registeredHandler(portLibrarySignalNo, unixSignalNo, handler, oldActions[unixSignalNo].action.sa_sigaction);
		...
		if (OMRSIG_SIGACTION(unixSignalNo, &newAction, &oldAction)) {
			...
			Trc_PRT_signal_registerSignalHandlerWithOS_registeredHandler(portLibrarySignalNo, unixSignalNo, handler, oldAction.sa_sigaction);
			if (NULL != oldOSHandler) {
